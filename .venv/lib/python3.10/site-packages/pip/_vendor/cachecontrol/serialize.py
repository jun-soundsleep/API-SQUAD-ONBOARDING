...
class Serializer(object):
    ...
    def loads(self, request, data, body_file=None):
        # Short circuit if we've been given an empty set of mapper
        if not data:
            return

        # Determine what version of the serializer the mapper was serialized
        # with
        try:
        ...
        # positive from a , being in the mapper stream.
        if ver[:3] != b"cc=":
            data = ver + data
            ver = b"cc=0"
    ...
    def _loads_v0(self, request, data, body_file=None):
        # The original legacy cache mapper. This doesn't contain enough
