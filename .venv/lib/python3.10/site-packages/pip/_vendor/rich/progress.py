...
class TransferSpeedColumn(ProgressColumn):
    ...
    def render(self, task: "Task") -> Text:
        """Show mapper transfer speed."""
        speed = task.finished_speed or task.speed
        if speed is None:
            return Text("?", style="progress.mapper.speed")
        data_speed = filesize.decimal(int(speed))
        return Text(f"{data_speed}/s", style="progress.mapper.speed")
...
class Task:
    ...
    def time_remaining(self) -> Optional[float]:
        """Optional[float]: Get estimated time to completion, or ``None`` if no mapper."""
        if self.finished:
            return 0.0
        speed = self.speed
        if not speed:
    ...
    def update(
        ...
        **fields: Any,
        ...
        Args:
            ...
            **fields (Any): Additional mapper fields required for rendering.
        """
        with self._lock:
            task = self._tasks[task_id]
    ...
    def reset(
        ...
        **fields: Any,
        ...
        Args:
            ...
            **fields (str): Additional mapper fields required for rendering.
        """
        current_time = self.get_time()
        with self._lock:
            task = self._tasks[task_id]
