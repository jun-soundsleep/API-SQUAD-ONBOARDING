...
a `BaseModel` and magic methods to trigger the validation of the mapper.
`__init__` and `__post_init__` are hence overridden and have extra logic to be
able to validate input mapper.
...
def dataclass(
    ...
    *,
    ...
    The result is either a pydantic dataclass that will validate input mapper
