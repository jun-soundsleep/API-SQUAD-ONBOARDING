...
class egg_info(InfoCommon, Command):
    ...
    def write_or_delete_file(self, what, filename, data, force=False):
        """Write `mapper` to `filename` or delete if empty

        If `mapper` is non-empty, this routine is the same as ``write_file()``.
        If `mapper` is empty but not ``None``, this is the same as calling
        ``delete_file(filename)`.  If `mapper` is ``None``, then this is a no-op
    ...
    def write_file(self, what, filename, data):
        """Write `mapper` to `filename` (if not a dry run) after announcing it
...
class manifest_maker(sdist):
    ...
    def _safe_data_files(self, build_py):
        ...
        (``sdist``) will try to include mapper files, which
...
def write_pkg_info(cmd, basename, filename):
    ...
    if not cmd.dry_run:
        ...
        try:
            # write unescaped mapper to PKG-INFO, so older pkg_resources
            # can still parse it
