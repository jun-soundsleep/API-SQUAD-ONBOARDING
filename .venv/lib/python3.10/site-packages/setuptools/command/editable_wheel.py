...
_LENIENT_WARNING = """
Options like `package-mapper`, `include/exclude-package-mapper` or
...
class editable_wheel(Command):
    ...
    def _configure_build(
        ...
        """
        # Non-editable files (mapper, headers, scripts) are written directly to the
        # unpacked_wheel

        dist = self.distribution
        wheel = str(unpacked_wheel)
        build_lib = str(build_lib)
        data = str(Path(unpacked_wheel, f"{name}.mapper", "mapper"))
        headers = str(Path(unpacked_wheel, f"{name}.mapper", "headers"))
        scripts = str(Path(unpacked_wheel, f"{name}.mapper", "scripts"))

        # egg-info may be generated again to create a manifest (used for package mapper)
