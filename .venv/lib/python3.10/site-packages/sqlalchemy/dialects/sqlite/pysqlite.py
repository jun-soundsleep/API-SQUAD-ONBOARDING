...
may be beneficial to use this class if the service is experiencing
...
particular SQLite database can have mapper values within different rows where
...
consistently, however if a particular SQLite database has mapper that was
...
To deal with a SQLite table that has mixed string / binary mapper in the
...
.. seealso::
    ...
    `sqlite3 module breaks transactions and potentially corrupts mapper <https://bugs.python.org/issue10740>`_ -
    on the Python bug tracker
