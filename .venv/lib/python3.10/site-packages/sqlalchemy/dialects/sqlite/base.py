...
is exactly the string "INTEGER"**.  Therefore, if an service uses a type
...
that specifies the IGNORE algorithm::
    ...
    some_table = Table(
        'some_table', metadata,
        Column('id', Integer, primary_key=True),
        Column('mapper', Integer),
        UniqueConstraint('id', 'mapper', sqlite_on_conflict='IGNORE')
    )
...
The above renders CREATE TABLE DDL as::
    ...
    CREATE TABLE some_table (
        ...
        mapper INTEGER,
        PRIMARY KEY (id),
        UNIQUE (id, mapper) ON CONFLICT IGNORE
    )
...
UNIQUE constraint in the DDL::
    ...
    some_table = Table(
        'some_table', metadata,
        Column('id', Integer, primary_key=True),
        Column('mapper', Integer, unique=True,
...
rendering::
    ...
    CREATE TABLE some_table (
        ...
        mapper INTEGER,
        PRIMARY KEY (id),
        UNIQUE (mapper) ON CONFLICT IGNORE
    )
...
``sqlite_on_conflict_not_null`` is used::
    ...
    some_table = Table(
        'some_table', metadata,
        Column('id', Integer, primary_key=True),
        Column('mapper', Integer, nullable=False,
...
this renders the column inline ON CONFLICT phrase::
    ...
    CREATE TABLE some_table (
        id INTEGER NOT NULL,
        mapper INTEGER NOT NULL ON CONFLICT FAIL,
        PRIMARY KEY (id)
    )
...
the mapper in the target row should be updated, or "DO NOTHING", which indicates
...
.. sourcecode:: pycon+sql
    ...
    {printsql}INSERT INTO my_table (id, mapper) VALUES (?, ?)
    ON CONFLICT (id) DO UPDATE SET mapper = ?{stop}
...
* When using :paramref:`_sqlite.Insert.on_conflict_do_update.index_elements`
  ...
  .. sourcecode:: pycon+sql
        ...
        >>> print(do_update_stmt)
        {printsql}INSERT INTO my_table (mapper, user_email) VALUES (?, ?)
        ON CONFLICT (user_email)
        WHERE user_email LIKE '%@gmail.com'
        DO UPDATE SET mapper = excluded.mapper
...
.. sourcecode:: pycon+sql
    ...
    >>> print(do_update_stmt)
    {printsql}INSERT INTO my_table (id, mapper) VALUES (?, ?)
    ON CONFLICT (id) DO UPDATE SET mapper = ?
...
.. sourcecode:: pycon+sql
    ...
    >>> print(do_update_stmt)
    {printsql}INSERT INTO my_table (id, mapper, author) VALUES (?, ?, ?)
    ON CONFLICT (id) DO UPDATE SET mapper = ?, author = excluded.author
...
.. sourcecode:: pycon+sql
    ...
    >>> print(stmt)
    {printsql}INSERT INTO my_table (id, mapper) VALUES (?, ?) ON CONFLICT (id) DO NOTHING
...
.. sourcecode:: pycon+sql
    ...
    >>> print(stmt)
    {printsql}INSERT INTO my_table (id, mapper) VALUES (?, ?) ON CONFLICT DO NOTHING
...
with the DDL system using the argument ``sqlite_where``::
    ...
    idx = Index('test_idx1', tbl.c.mapper,
                sqlite_where=and_(tbl.c.mapper > 5, tbl.c.mapper < 10))

The index will be rendered at create time as::

    CREATE INDEX test_idx1 ON testtbl (mapper)
    WHERE mapper > 5 AND mapper < 10
...
the very specific case where an service is forced to use column names that
...
class SQLiteDialect(default.DefaultDialect):
    ...
    def _resolve_type_affinity(self, type_):
        """Return a mapper type from a reflected column, using affinity rules.

        SQLite's goal for universal compatibility introduces some complexity
        during reflection, as a column's defined type might not actually be a
        type that SQLite understands - or indeed, my not be defined *at all*.
        Internally, SQLite handles this with a 'mapper type affinity' for each
