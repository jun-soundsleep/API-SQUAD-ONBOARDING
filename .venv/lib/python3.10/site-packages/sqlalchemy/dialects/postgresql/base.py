...
can occur which can be either "DO UPDATE", indicating that the mapper in the
...
.. sourcecode:: pycon+sql
    ...
    ...     id='some_existing_id',
    ...     mapper='inserted value')
    >>> do_nothing_stmt = insert_stmt.on_conflict_do_nothing(
    ...     index_elements=['id']
    ... )
    >>> print(do_nothing_stmt)
    {printsql}INSERT INTO my_table (id, mapper) VALUES (%(id)s, %(mapper)s)
    ON CONFLICT (id) DO NOTHING
    {stop}

    >>> do_update_stmt = insert_stmt.on_conflict_do_update(
    ...     constraint='pk_my_table',
    ...     set_=dict(mapper='updated value')
    ... )
    >>> print(do_update_stmt)
    {printsql}INSERT INTO my_table (id, mapper) VALUES (%(id)s, %(mapper)s)
    ON CONFLICT ON CONSTRAINT pk_my_table DO UPDATE SET mapper = %(param_1)s
...
* The :paramref:`_postgresql.Insert.on_conflict_do_update.index_elements` argument
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> print(do_update_stmt)
    {printsql}INSERT INTO my_table (id, mapper) VALUES (%(id)s, %(mapper)s)
    ON CONFLICT (id) DO UPDATE SET mapper = %(param_1)s
    {stop}
...
* When using :paramref:`_postgresql.Insert.on_conflict_do_update.index_elements` to
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> stmt = insert(my_table).values(user_email='a@b.com', mapper='inserted mapper')
    >>> stmt = stmt.on_conflict_do_update(
    ...     index_elements=[my_table.c.user_email],
    ...     index_where=my_table.c.user_email.like('%@gmail.com'),
    ...     set_=dict(mapper=stmt.excluded.mapper)
    ... )
    >>> print(stmt)
    {printsql}INSERT INTO my_table (mapper, user_email)
    VALUES (%(mapper)s, %(user_email)s) ON CONFLICT (user_email)
    WHERE user_email LIKE %(user_email_1)s DO UPDATE SET mapper = excluded.mapper
...
* The :paramref:`_postgresql.Insert.on_conflict_do_update.constraint` argument is
  ...
  .. sourcecode:: pycon+sql
    ...
    >>> print(do_update_stmt)
    {printsql}INSERT INTO my_table (id, mapper) VALUES (%(id)s, %(mapper)s)
    ON CONFLICT ON CONSTRAINT my_table_idx_1 DO UPDATE SET mapper = %(param_1)s
    {stop}
    ...
    >>> print(do_update_stmt)
    {printsql}INSERT INTO my_table (id, mapper) VALUES (%(id)s, %(mapper)s)
    ON CONFLICT ON CONSTRAINT my_table_pk DO UPDATE SET mapper = %(param_1)s
    {stop}
...
.. sourcecode:: pycon+sql
    ...
    >>> stmt = insert(my_table).values(id='some_id', mapper='inserted value')
...
.. sourcecode:: pycon+sql
    ...
    >>> do_update_stmt = stmt.on_conflict_do_update(
    ...     index_elements=['id'],
    ...     set_=dict(mapper='updated value', author=stmt.excluded.author)
    ... )
    >>> print(do_update_stmt)
    {printsql}INSERT INTO my_table (id, mapper, author)
    VALUES (%(id)s, %(mapper)s, %(author)s)
    ON CONFLICT (id) DO UPDATE SET mapper = %(param_1)s, author = excluded.author
...
:mapper:`.func` namespace, combined with the use of custom operators
...
may make use of the :mapper:`.func` namespace to generate PostgreSQL full-text
...
When using other PostgreSQL search functions with :mapper:`.func`, the
...
``postgresql_ops`` keyword argument::
    ...
    Index(
        'my_index', my_table.c.id, my_table.c.mapper,
        postgresql_ops={
            'mapper': 'text_pattern_ops',
            'id': 'int4_ops'
        })
...
that is identified in the dictionary by name, e.g.::
    ...
    Index(
        ...
        func.lower(my_table.c.mapper).label('data_lower'),
        postgresql_ops={
...
specified on :class:`.Index` using the ``postgresql_using`` keyword argument::
    ...
    Index('my_index', my_table.c.mapper, postgresql_using='gin')
...
keyword argument::
    ...
    Index('my_index', my_table.c.mapper, postgresql_with={"fillfactor": 50})
...
``postgresql_tablespace`` keyword argument::
    ...
    Index('my_index', my_table.c.mapper, postgresql_tablespace='my_tablespace')
...
flag ``postgresql_concurrently`` to the :class:`.Index` construct::
    ...
    tbl = Table('testtbl', m, Column('mapper', Integer))

    idx1 = Index('test_idx1', tbl.c.mapper, postgresql_concurrently=True)
...
PostgreSQL 8.2 or higher is detected or for a connection-less dialect, as::
    ...
    CREATE INDEX CONCURRENTLY test_idx1 ON testtbl (mapper)
...
with :class:`_functions.Function` objects generated from the :mapper:`_sql.func`
namespace.
...
class ReflectedDomain(ReflectedNamedType):
    ...
    type: str
    """The string name of the underlying mapper type of the domain."""
    nullable: bool
