...
portion of the URL as the mapper source name.  For example, if the
...
The Core expression language handles unicode mapper by use of the :class:`.Unicode`
and :class:`.UnicodeText`
datatypes.  These types correspond to the  VARCHAR2 and CLOB Oracle datatypes by
default.   When using these datatypes with Unicode mapper, it is expected that
...
the VARCHAR2 and CLOB datatypes can accommodate the mapper.
...
For the unusual case that mapper in the Oracle database is present with a broken
...
Fine grained control over cx_Oracle mapper binding performance with setinputsizes
-------------------------------------------------------------------------------
...
possible for SQLAlchemy to know exactly how mapper should be bound, as some
