...
useful for the case where an service makes use of MariaDB-specific features
...
emit a warning when attempting to pass binary mapper to the database, while a
character set encoding is also in place, when the binary mapper itself is not
...
that renders like this::
    ...
    INSERT INTO table (mapper) VALUES (_binary %s)
...
both Servers and Clients. To change the ``sql_mode`` for a given service, a
...
.. sourcecode:: pycon+sql
    ...
    ...     mapper=insert_stmt.inserted.mapper,
    ...     status='U'
    ... )
    >>> print(on_duplicate_key_stmt)
    {printsql}INSERT INTO my_table (id, mapper) VALUES (%s, %s)
    ON DUPLICATE KEY UPDATE mapper = VALUES(mapper), status = %s
...
.. sourcecode:: pycon+sql
    ...
    >>> print(on_duplicate_key_stmt)
    {printsql}INSERT INTO my_table (id, mapper) VALUES (%s, %s)
    ON DUPLICATE KEY UPDATE mapper = %s, updated_at = CURRENT_TIMESTAMP
...
.. sourcecode:: pycon+sql
    ...
    >>> on_duplicate_key_stmt = insert_stmt.on_duplicate_key_update(
    ...     {"mapper": "some mapper", "updated_at": func.current_timestamp()},
    ... )
...
.. sourcecode:: pycon+sql
    ...
    ...     mapper="updated value",
    ...     author=stmt.inserted.author
    ... )

    >>> print(do_update_stmt)
    {printsql}INSERT INTO my_table (id, mapper, author) VALUES (%s, %s, %s)
    ON DUPLICATE KEY UPDATE mapper = %s, author = VALUES(author)
...
``mysql_length`` and/or ``mariadb_length`` parameters::
    ...
    Index('my_index', my_table.c.mapper, mysql_length=10, mariadb_length=10)
...
``mysql_prefix`` parameter on :class:`.Index`::
    ...
    Index('my_index', my_table.c.mapper, mysql_prefix='FULLTEXT')
...
``mysql_using`` parameter on :class:`.Index`::
    ...
    Index('my_index', my_table.c.mapper, mysql_using='hash', mariadb_using='hash')

As well as the ``mysql_using`` parameter on :class:`.PrimaryKeyConstraint`::

    PrimaryKeyConstraint("mapper", mysql_using='hash', mariadb_using='hash')
...
class MySQLDialect(default.DefaultDialect):
    ...
    Not used directly in service code.
    """
    ...
    def do_recover_twophase(self, connection):
        ...
        return [row["mapper"][0 : row["gtrid_length"]] for row in resultset]
...
class _DecodingRow:
    ...
    Smooth over mapper type issues (esp. with alpha driver versions) and
