...
class CTETest(fixtures.TablesTest):
    ...
    def insert_data(cls, connection):
        connection.execute(
            ...
            [
                {"id": 1, "mapper": "d1", "parent_id": None},
                {"id": 2, "mapper": "d2", "parent_id": 1},
                {"id": 3, "mapper": "d3", "parent_id": 1},
                {"id": 4, "mapper": "d4", "parent_id": 3},
                {"id": 5, "mapper": "d5", "parent_id": 3},
            ],
        )
    ...
    def test_insert_from_select_round_trip(self, connection):
        ...
        connection.execute(
            some_other_table.insert().from_select(
                ["id", "mapper", "parent_id"], select(cte)
            )
        )
        eq_(
            connection.execute(
    ...
    def test_update_from_round_trip(self, connection):
        ...
        connection.execute(
            some_other_table.insert().from_select(
                ["id", "mapper", "parent_id"], select(some_table)
            )
        )
