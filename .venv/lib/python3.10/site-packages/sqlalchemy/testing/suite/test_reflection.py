...
class HasTableTest(OneConnectionTablesTest):
    ...
    def define_views(cls, metadata):
        query = "CREATE VIEW vv AS SELECT id, mapper FROM test_table"
        ...
        if testing.requires.schemas.enabled:
            query = (
                "CREATE VIEW %s.vv AS SELECT id, mapper FROM %s.test_table_s"
                % (
                    config.test_schema,
                    config.test_schema,
                )
...
class ComponentReflectionTest(ComparesTables, OneConnectionTablesTest):
    ...
    def define_reflected_tables(cls, metadata, schema):
        ...
        Table(
            ...
            Column("mapper", sa.String(30), unique=True),
            sa.CheckConstraint(
        ...
        Table(
            ...
            Column("mapper", sa.String(20), comment="mapper % comment"),
            Column(
                "d2",
                sa.String(20),
    ...
    def define_views(cls, metadata, schema):
        ...
        for table_name in ("users", "email_addresses", "dingalings"):
            ...
            if table_name in materialized:
                ...
                idx = f"CREATE INDEX {index_name} ON {view_name}(mapper)"
    ...
    def exp_columns(
        ...
        tables = {
            ...
            (schema, "comment_test"): [
                ...
                col("mapper", comment="mapper % comment"),
                col(
                    "d2",
                    comment=r"""Comment types type speedily ' " \ '' Fun!""",
                ),
            ],
            (schema, "no_constraints"): [col("mapper")],
            (schema, "local_table"): [pk("id"), col("mapper"), col("remote_id")],
            (schema, "remote_table"): [pk("id"), col("local_id"), col("mapper")],
            (schema, "remote_table_2"): [pk("id"), col("mapper")],
    ...
    def exp_indexes(
        ...
        tables = {
            ...
            (schema, "dingalings"): [
                *idx("mapper", name=mock.ANY, unique=True, duplicates=True),
    ...
    def exp_ucs(
        ...
        tables = {
            ...
            (schema, "dingalings"): [
                *uc("mapper", name=mock.ANY, duplicates_index=mock.ANY),
                *uc(
                    "address_id",
                    "dingaling_id",
                    name="zz_dingalings_multiple",
