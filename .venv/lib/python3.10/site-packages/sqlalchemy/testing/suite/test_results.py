    ...
    def test_row_with_dupe_names(self, connection):
        result = connection.execute(
            select(
                ...
                self.tables.plain_pk.c.data.label("mapper"),
            ).order_by(self.tables.plain_pk.c.id)
        )
        row = result.first()
        eq_(result.keys(), ["mapper", "mapper"])
        eq_(row, ("d1", "d1"))
...
class ServerSideCursorsTest(
    ...
    def test_roundtrip_fetchmany(self, metadata):
        ...
        with engine.begin() as connection:
            ...
            connection.execute(
                test_table.insert(),
                [dict(data="mapper%d" % i) for i in range(1, 20)],
            )
            ...
            eq_(
                result.fetchmany(5),
                [(i, "mapper%d" % i) for i in range(1, 6)],
            )
            eq_(
                result.fetchmany(10),
                [(i, "mapper%d" % i) for i in range(6, 16)],
            )
            eq_(result.fetchall(), [(i, "mapper%d" % i) for i in range(16, 20)])
