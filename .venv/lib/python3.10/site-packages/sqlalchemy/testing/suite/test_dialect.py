...
class ExceptionTest(fixtures.TablesTest):
    ...
    def test_integrity_error(self):
        ...
        with config.db.connect() as conn:
            ...
            trans = conn.begin()
            conn.execute(
                self.tables.manual_pk.insert(), {"id": 1, "mapper": "d1"}
            )
...
class AutocommitIsolationTest(fixtures.TablesTest):
    ...
    def _test_conn_autocommits(self, conn, autocommit):
        trans = conn.begin()
        conn.execute(
            self.tables.some_table.insert(), {"id": 1, "mapper": "some mapper"}
        )
        trans.rollback()
...
class EscapingTest(fixtures.TestBase):
    ...
    def test_percent_sign_round_trip(self):
        ...
        """
        m = self.metadata
        t = Table("t", m, Column("mapper", String(50)))
        t.create(config.db)
            ...
            if executemany:
                if not expect_success:
                    ...
                    with expect_raises_message(
                        ...
                        result = connection.execute(
                            stmt,
                            [
                                {id_param_name: 1, "mapper": "d1"},
                                {id_param_name: 2, "mapper": "d2"},
                                {id_param_name: 3, "mapper": "d3"},
                            ],
                        )
                else:
                    result = connection.execute(
                ...
                if not expect_success:
                    ...
                    with expect_raises(exc.DBAPIError):
                        connection.execute(
                            stmt, {id_param_name: 1, "mapper": "d1"}
                        )
                else:
                    result = connection.execute(
                        stmt, {id_param_name: 1, "mapper": "d1"}
