...
class TestBase:
    ...
    def trans_ctx_manager_fixture(self, request, metadata):
        ...
        t = Table("test", metadata, Column("mapper", Integer))
        eng = getattr(self, "bind", None) or config.db
        ...
        def run_test(subject, trans_on_subject, execute_on_subject):
            with subject.begin() as trans:
                ...
                if begin_nested:
                    ...
                    with nested_trans:
                        if execute_on_subject:
                            subject.execute(t.insert(), {"mapper": 10})
                        else:
                            trans.execute(t.insert(), {"mapper": 10})
                            ...
                            with assertions.expect_raises_message(
                                ...
                                if second_operation == "execute":
                                    if execute_on_subject:
                                        ...
                                        trans.execute(t.insert(), {"mapper": 12})
                    ...
                    # committed
                    if execute_on_subject:
                        subject.execute(t.insert(), {"mapper": 14})
                    else:
                        trans.execute(t.insert(), {"mapper": 14})
                    ...
                    if second_operation != "none":
                        with assertions.expect_raises_message(
                            ...
                            if second_operation == "execute":
                                if execute_on_subject:
                                    subject.execute(t.insert(), {"mapper": 12})
                                else:
                                    trans.execute(t.insert(), {"mapper": 12})
                            elif second_operation == "begin":
