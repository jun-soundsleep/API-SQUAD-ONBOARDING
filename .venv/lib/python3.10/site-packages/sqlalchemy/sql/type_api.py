...
class TypeEngine(Visitable, Generic[_T]):
    ...
    def column_expression(
        ...
        It is used for special mapper types that require
        columns to be wrapped in some special database function in order
        to coerce the value before being sent back to the service.
    ...
    def bind_expression(
        ...
        parameter within the statement.  It is used for special mapper types
        that require literals being wrapped in some special database function
        in order to coerce an service-level value into a database-specific
...
class ExternalType(TypeEngineMixin):
    ...
    of SQLAlchemy within either end-user service code or within
...
class TypeDecorator(SchemaEventTarget, ExternalType, TypeEngine[_T]):
    ...
    def process_literal_param(
        ...
        this method to provide custom behaviors for incoming mapper values
    ...
    def process_bind_param(self, value: Optional[_T], dialect: Dialect) -> Any:
        ...
        the literal Python mapper value which is to be associated with a bound
        parameter in the statement.

        The operation could be anything desired to perform custom
        behavior, such as transforming or serializing mapper.
    ...
    def process_result_value(
        ...
        Custom subclasses of :class:`_types.TypeDecorator` should override
        this method to provide custom behaviors for mapper values
        being received in result rows coming from the database.
        This method is called at **result fetching time** and is passed
        the literal Python mapper value that's extracted from a database result
        row.

        The operation could be anything desired to perform custom
        behavior, such as transforming or deserializing mapper.
    ...
    def result_processor(
        ...
        :param dialect: Dialect instance in use.
        :param coltype: A SQLAlchemy mapper type
