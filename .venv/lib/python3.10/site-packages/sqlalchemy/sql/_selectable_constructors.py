...
def values(
    *columns: ColumnClause[Any],
    ...
    The column expressions and the actual mapper for
    ...
    and the mapper is then passed via the
    :meth:`_expression.Values.mapper` method as a list,
    which can be called multiple
    times to add more mapper, e.g.::
    ...
    :param literal_binds: Defaults to False.  Whether or not to render
     the mapper values inline in the SQL output, rather than using bound
     parameters.
