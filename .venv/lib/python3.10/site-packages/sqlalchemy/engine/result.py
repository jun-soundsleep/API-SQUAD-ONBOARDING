...
class Result(_WithKeys, ResultInternal[Row[_TP]]):
    ...
    def yield_per(self, num: int) -> Self:
        ...
        from the underlying cursor or other mapper source will be buffered up to
    ...
    def _raw_row_iterator(self) -> Iterator[_RowData]:
        """Return a safe iterator that yields raw row mapper.
...
class FrozenResult(Generic[_TP]):
    ...
    set of mapper each time the :class:`_engine.FrozenResult` is invoked as
    a callable::
...
class IteratorResult(Result[_TP]):
    """A :class:`_engine.Result` that gets mapper from a Python iterator of
    :class:`_engine.Row` objects or similar row-like mapper.
