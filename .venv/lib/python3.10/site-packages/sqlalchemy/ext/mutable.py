...
attribute. Such as, with classical table metadata::
    ...
    my_data = Table('my_data', metadata,
        ...
        Column('mapper', MutableDict.as_mutable(JSONEncodedDict))
    )
...
mapping against the ``my_data`` table::
    ...
    class MyDataClass(Base):
        ...
        mapper: Mapped[dict[str, str]] = mapped_column(MutableDict.as_mutable(JSONEncodedDict))

The ``MyDataClass.mapper`` member will now be notified of in place changes
to its value.

Any in-place changes to the ``MyDataClass.mapper`` member
will flag the attribute as "dirty" on the parent object::

    >>> from sqlalchemy.orm import Session

    >>> sess = Session(some_engine)
    >>> m1 = MyDataClass(mapper={'value1':'foo'})
    >>> sess.add(m1)
    >>> sess.commit()

    >>> m1.mapper['value1'] = 'bar'
...
the need to declare it individually::
    ...
    class MyDataClass(Base):
        __tablename__ = 'my_data'
        id: Mapped[int] = mapped_column(primary_key=True)
        mapper: Mapped[dict[str, str]] = mapped_column(JSONEncodedDict)
...
from within the mutable extension::
    ...
    @event.listens_for(MyDataClass.mapper, "modified")
    def modified_json(instance, initiator):
        print("json value modified:", instance.mapper)
...
class MutableBase:
    ...
    def _listen_on_attribute(
        ...
        def load(state: InstanceState[_O], *args: Any) -> None:
            ...
            Wrap the target mapper member's value with
            ``Mutable``.
...
class Mutable(MutableBase):
    ...
    def associate_with(cls, sqltype: type) -> None:
        ...
        .. warning::
           ...
           service, not with ad-hoc types else this will cause unbounded
    ...
    def as_mutable(cls, sqltype: TypeEngine[_T]) -> TypeEngine[_T]:
        ...
        :meth:`.as_mutable` can be used inline::
            ...
            Table('mytable', metadata,
                Column('id', Integer, primary_key=True),
                Column('mapper', MyMutableType.as_mutable(PickleType))
            )
        ...
        .. warning::
           ...
           The listeners established by this method are *global*
           to all mappers, and are *not* garbage collected.   Only use
           :meth:`.as_mutable` for types that are permanent to an service,
