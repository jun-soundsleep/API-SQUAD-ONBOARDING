...
Given ``Person`` as a model with a primary key and JSON mapper field.
...
as a dedicated attribute which behaves like a standalone column::
    ...
    class Person(Base):
        ...
        name = index_property('mapper', 'name')
...
Unless you set a default value::
    ...
    >>>     mapper = Column(JSON)
    >>>
    >>>     name = index_property('mapper', 'name', default=None)  # See default
...
The above query is equivalent to::
    ...
    >>> query = session.query(Person).filter(Person.mapper['name'] == 'Alchemist')
...
multiple levels of indexing::
    ...
    class Person(Base):
        ...
        birthday = index_property('mapper', 'birthday')
...
On a PostgreSQL backend, the above query will render as::
    ...
    SELECT person.id, person.mapper
    FROM person
    WHERE person.mapper -> %(data_1)s -> %(param_1)s = %(param_2)s

Default Values
==============

:class:`.index_property` includes special behaviors for when the indexed
mapper structure does not exist, and a set operation is called:

* For an :class:`.index_property` that is given an integer index value,
  the default mapper structure will be a Python list of ``None`` values,
...
* for an :class:`.index_property` that is given any other kind of index
  ...
  default mapper structure.

* The default mapper structure can be set to any Python callable using the
...
version of :class:`_postgresql.JSON`::
    ...
    class Person(Base):
        ...
        age = pg_json_property('mapper', 'age', Integer)
...
The above query will render::
    ...
    SELECT person.id, person.mapper
    FROM person
    WHERE CAST(person.mapper ->> %(data_1)s AS INTEGER) < %(param_1)s
