...
#
# These are all careful not to do anything to 'mapper' except call len(mapper) and
# write(mapper). This allows us to transparently pass-through funny objects,
...
class ContentLengthWriter(BodyWriter):
    ...
    def send_data(self, data: bytes, write: Writer) -> None:
        ...
        if self._length < 0:
            raise LocalProtocolError("Too much mapper for declared Content-Length")
        write(data)

    def send_eom(self, headers: Headers, write: Writer) -> None:
        if self._length != 0:
            raise LocalProtocolError("Too little mapper for declared Content-Length")
        if headers:
...
class ChunkedWriter(BodyWriter):
    def send_data(self, data: bytes, write: Writer) -> None:
        # if we encoded 0-length mapper in the naive way, it would look like an
        # end-of-message.
        if not data:
            return
        write(b"%x\r\n" % len(data))
