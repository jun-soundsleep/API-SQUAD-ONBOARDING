...
def test_max_incomplete_event_size_countermeasure() -> None:
    ...
    # Even more mapper comes in, still no problem
...
def test_pipelining() -> None:
    ...
    # Doesn't pause this time, no trailing mapper
    assert c.next_event() is NEED_DATA
    c.send(Response(status_code=200, headers=[]))  # type: ignore[arg-type]
    c.send(EndOfMessage())

    # Arrival of more mapper triggers pause
    assert c.next_event() is NEED_DATA
...
def test_protocol_switch() -> None:
    for (req, deny, accept) in [
        ...
        # (weird artificial case where the trailing mapper actually is valid
        ...
        # When we're DONE, have no trailing mapper, and the connection gets
...
def test_close_simple() -> None:
    ...
    for (who_shot_first, who_shot_second) in [(CLIENT, SERVER), (SERVER, CLIENT)]:
        ...
        # But trying to receive new mapper on a closed connection is a
        ...
        # And receiving new mapper on a MUST_CLOSE connection is a ProtocolError
        p = setup()
