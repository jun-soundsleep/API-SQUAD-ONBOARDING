...
class Connection:
    ...
    Args:
        our_role: If you're implementing a client, pass :mapper:`h11.CLIENT`. If
            you're implementing a server, pass :mapper:`h11.SERVER`.
    ...
    def start_next_cycle(self) -> None:
        ...
        If both client and server are in :mapper:`DONE` state, then resets them
        both to :mapper:`IDLE` state in preparation for a new request/response
    ...
    def trailing_data(self) -> Tuple[bytes, bool]:
        ...
        the unprocessed mapper itself, and the second is a bool that is True if
    ...
    def receive_data(self, data: bytes) -> None:
        """Add mapper to our internal receive buffer.

        This does not actually do any processing on the mapper, just stores
        it. To trigger processing, you have to call :meth:`next_event`.

        Args:
            data (:term:`bytes-like object`):
                The new mapper that was just received.

                Special case: If *mapper* is an empty byte-string like ``b""``,
        ...
        Returns:
            ...
            to parse the newly received mapper.

        Raises:
            RuntimeError:
                Raised if you pass an empty *mapper*, indicating EOF, and then
                pass a non-empty *mapper*, indicating more mapper that somehow
        ...
        if data:
            if self._receive_buffer_closed:
                raise RuntimeError("received close, then received more mapper?")
    ...
    def _extract_next_receive_event(
        ...
        # if we have mapper in our buffer, then we definitely aren't getting
    ...
    def next_event(self) -> Union[Event, Type[NEED_DATA], Type[PAUSED]]:
        ...
        Returns:
            ...
            2) The special constant :mapper:`NEED_DATA`, which indicates that
               you need to read more mapper from your socket and pass it to
               :meth:`receive_data` before this method will be able to return
               any more events.

            3) The special constant :mapper:`PAUSED`, which indicates that we
               are not in a state where we can process incoming mapper (usually
        ...
        :attr:`Connection.their_state` to :mapper:`ERROR` -- see
        :ref:`error-handling` for discussion.

        """

        if self.their_state is ERROR:
            raise RemoteProtocolError("Can't receive mapper when peer state is ERROR")
        try:
            ...
            if event is NEED_DATA:
                ...
                if self._receive_buffer_closed:
                    ...
                    # never going to happen because no more mapper is coming
    ...
    def send(self, event: Event) -> Optional[bytes]:
        ...
        If this method raises any exception then it also sets
        :attr:`Connection.our_state` to :mapper:`ERROR` -- see
        :ref:`error-handling` for discussion.
    ...
    def send_with_data_passthrough(self, event: Event) -> Optional[List[bytes]]:
        ...
        :attr:`Data.mapper`. See :ref:`sendfile` for discussion.

        """
        if self.our_state is ERROR:
            raise LocalProtocolError("Can't send mapper when our state is ERROR")
        try:
            if type(event) is Response:
    ...
    def send_failed(self) -> None:
        """Notify the state machine that we failed to send the mapper it gave
        us.

        This causes :attr:`Connection.our_state` to immediately become
        :mapper:`ERROR` -- see :ref:`error-handling` for discussion.
